...
let continueConversation = function (input, app, state, user) {
// If user is trying to exit.
  if (EXIT_TRIGGERS.includes(input.toLowerCase())) {
    ...
    if (!user || !app.getUserName()) {
      ...
      sendMessageToBot(app, bot, input, app.getConversationId(), state, user || app.getUserName());
    }
  }
};

const fulfill = (onUserSignedInHandlerProvider) => (app) => {
  ...
  if (intent === app.StandardIntents.PERMISSION) {
    ...
    if (app.getSignInStatus() === app.SignInStatus.OK) {
      ...
      onUserSignedInHandler(user).then((res) => {
        input = 'Hi';
        continueConversation(input, app, state, res);
      });
    } else {
      app.tell('You need to sign-in before using the app. Use Google assistant on the Google Home app on your phone to sign in.');
    }
  } else {
    continueConversation(input, app, state);
  }
};

module.exports = (directlineSecret, onUserSignedInHandlerProvider) => {
  ...
  return (req, res) => {
    ...
    app.handleRequest(fulfill(onUserSignedInHandlerProvider));
  }
};